...
export default defineComponent({
  ...
  setup() {
    ...
    const openProjectDetail = (project: Project) => {
      if (!openProjects.value.find((p) => p.id === project.id)) {
        ...
        setTimeout(() => {
          ...
          if (projectWindow) {
            projectWindow.addEventListener("mousedown", handleWindowMouseDown);
          }
        }, 0);
      }
    };
    ...
    const closeProjectDetail = (projectId: number) => {
      ...
      if (projectWindow) {
        projectWindow.removeEventListener("mousedown", handleWindowMouseDown);
      }
    ...
    const closeApp = (appName: string) => {
      ...
      if (highestZIndexApp) {
        currentActiveWindow.value =
          apps.find((app) => app.nickName === highestZIndexApp[0])?.name ||
          "Finder";
      } else {
    ...
    function initDragSelect() {
      ...
      if (area instanceof HTMLElement) {
        ...
        setTimeout(() => {
          ds = new DragSelect({
            selectables: Array.from(document.querySelectorAll(".desktop-icon")),
            area: area,
    ...
    function handleWindowMouseDown(e: Event) {
      ...
      const appName = (e.currentTarget as HTMLElement).getAttribute("data-app");
      if (appName) {
        updateZIndex(appName);
        const app = apps.find((a) => a.nickName === appName);
        if (app) {
